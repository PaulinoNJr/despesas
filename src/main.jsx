...
function CreditCardBills({ finance, openModal }) {
  ...
  const readInvoice = async event => {
    ...
    try {
      ...
      openModal({ type: 'review-card-invoice', review: { invoice, fileName: file.name } })
      setImportMessage('')
...
function CardInvoiceReviewModal({ finance, initialReview, onClose }) {
  ...
  const [message, setMessage] = useState('')
  const [cardId, setCardId] = useState(() => finance.cards[0]?.id || '')
  const [saving, setSaving] = useState(false)
  ...
  return <Modal title="Conferir fatura do cartão" onClose={onClose}><div className="invoice-import"><div className="invoice-summary"><div><span>IDENTIFICADOR</span><strong>{String(invoice.invoiceKey || 'Fatura importada')}</strong></div><div><span>VENCIMENTO</span><strong>{String(invoice.dueDate || 'Não identificado')}</strong></div><div><span>TOTAL DA FATURA</span><strong>{money(invoiceTotal)}</strong></div></div><label>Cartão cadastrado<select value={cardId} onChange={event => setCardId(event.target.value)} required><option value="">Selecione um cartão</option>{finance.cards.map(card => <option key={card.id} value={card.id}>{card.name} · vence dia {card.dueDay}</option>)}</select></label><div className="invoice-review-head"><strong>Lançamentos identificados</strong><span className={isExact ? 'exact' : 'not-exact'}>{isExact ? 'Total confere' : `Diferença de ${money(invoiceTotal - selectedTotal)}`}</span></div><div className="invoice-entry-list">{entries.map(entry => <div className="invoice-entry" key={entry.id}><input type="checkbox" checked={Boolean(entry.included)} onChange={event => updateEntry(entry.id, { included: event.target.checked })}/><div><input value={String(entry.description)} onChange={event => updateEntry(entry.id, { description: event.target.value })}/><small>{entry.date || 'Ajuste da fatura'}{entry.installmentLabel ? ` · parcela ${entry.installmentLabel}` : ''}</small></div><input className="invoice-value" type="number" step="0.01" value={Number(entry.value) || 0} onChange={event => updateEntry(entry.id, { value: Number(event.target.value) || 0 })}/><label className="invoice-installments">Restam<input type="number" min="1" max="360" value={Math.max(1, Number(entry.installmentsLeft) || 1)} onChange={event => updateEntry(entry.id, { installmentsLeft: Number(event.target.value) || 1 })}/></label></div>)}</div>{message && <div className="login-error">{message}</div>}<p className="modal-note">Revise os lançamentos. O cartão escolhido define o vencimento usado em cada compra importada.</p><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={!isExact || saving || !cardId} onClick={confirm}>{saving ? 'Importando…' : 'Confirmar e importar'}</button></div></div></Modal>
}
