...
function useFinance() {
  ...
  const importCardInvoice = async ({ invoice, bills }) => {
    ...
    const payload = nextBills.map(bill => ({ id: bill.id, name: bill.name, value: bill.value, due_day: bill.dueDay, type: bill.type, category: bill.category, responsible: bill.responsible || null, installments: bill.installments || null, start_period: bill.startPeriod || null, flow: 'payable', is_credit_card: true, card_id: bill.cardId || null, card_name: bill.cardName, card_invoice_id: invoice.id }))
  ...
  return { ...data, invoices, family, remote, user, authReady, connectionError, preferences, signIn, signInWithPasskey, signOut, registerPasskey, removePasskey, changePassword, updateDisplayName, updatePreferences, inviteFamilyMember, cancelFamilyInvite, getFamilyInvitation, acceptFamilyInvitation, addPerson, addIncome, addCategory, addType, addCard, addBill, updateBill, importCardInvoice, removeCardInvoice, toggleBill, getStatus, remove }
}
...
function Bills({ finance, openModal }) {
  ...
  const bills = finance.bills
    ...
    .filter(bill => !isPayable(bill) || isBillActiveInPeriod(bill, period))
    .map(bill => {
      const scheduledPeriod = !isPayable(bill) && bill.startPeriod ? bill.startPeriod : period
...
function BillModal({ people, categories, types, cards = [], initialPeriod, initialBill, forceCreditCard, onClose, onSave }) {
  const [form, setForm] = useState(() => initialBill ? { name: initialBill.name, value: initialBill.value, dueDay: String(initialBill.dueDay), type: initialBill.type, category: initialBill.category, responsible: initialBill.responsible || '', installments: initialBill.isCreditCard ? (initialBill.installments || 1) : initialBill.installments || '', startPeriod: initialBill.startPeriod || initialPeriod, flow: initialBill.flow || 'payable', receivableMode: initialBill.flow === 'receivable' && !initialBill.installments ? 'recurring' : 'once', isCreditCard: Boolean(initialBill.isCreditCard), cardId: initialBill.cardId || cards.find(card => card.name === initialBill.cardName)?.id || '', cardInvoiceId: initialBill.cardInvoiceId || null } : { name: '', value: '', dueDay: '10', type: types[0]?.name || '', category: categories[0]?.name || '', responsible: people[0]?.id || '', installments: forceCreditCard ? 1 : '', startPeriod: initialPeriod, flow: 'payable', receivableMode: 'once', isCreditCard: Boolean(forceCreditCard), cardId: forceCreditCard ? cards[0]?.id || '' : '', cardInvoiceId: null })
  ...
  const submit = async event => {
    ...
    const error = await onSave({ ...form, id: initialBill?.id || crypto.randomUUID(), cardId: creditCard ? selectedCard?.id || null : null, cardName: creditCard ? selectedCard?.name || '' : '', flow: creditCard ? 'payable' : form.flow, isCreditCard: creditCard, value: Number(form.value), dueDay: receivable ? 1 : creditCard ? selectedCard.dueDay : Number(form.dueDay), type: receivable ? 'Recebimento' : form.type, category: receivable ? 'Receitas' : form.category, responsible: receivable ? '' : form.responsible, installments: receivable ? (form.receivableMode === 'recurring' ? null : 1) : creditCard ? Math.max(1, Number(form.installments) || 1) : form.installments ? Number(form.installments) : null, startPeriod: receivable ? (form.receivableMode === 'recurring' ? null : form.startPeriod || initialPeriod) : creditCard ? form.startPeriod || initialPeriod : form.installments ? form.startPeriod : null, status: initialBill?.status || 'pending' })
    if (error) window.alert(error)
  }
  const flowField = <><label>Fluxo<select disabled={creditCard} value={creditCard ? 'payable' : form.flow} onChange={event => setForm({ ...form, flow: event.target.value, receivableMode: event.target.value === 'receivable' ? 'once' : form.receivableMode, installments: event.target.value === 'receivable' ? 1 : '', startPeriod: event.target.value === 'receivable' ? initialPeriod : form.startPeriod })}><option value="payable">Conta a pagar</option><option value="receivable">Conta a receber</option></select></label>{creditCard && <label>Cartão<select required value={form.cardId} onChange={event => setForm({ ...form, cardId: event.target.value })}><option value="">Selecione um cartão</option>{cards.map(card => <option key={card.id} value={card.id}>{card.name} · vence dia {card.dueDay}</option>)}</select></label>}</>
  const selectedCard = cards.find(card => card.id === form.cardId)
  return <Modal title={initialBill ? 'Editar lançamento' : creditCard ? 'Nova compra no cartão' : 'Novo lançamento'} onClose={onClose}><form onSubmit={submit}>{receivable ? flowField : <div className="form-row">{flowField}{!creditCard && <label>Vencimento<input required min="1" max="31" type="number" value={form.dueDay} onChange={event => setForm({ ...form, dueDay: event.target.value })}/></label>}{creditCard && <p className="modal-note">Vencimento: dia {selectedCard?.dueDay || '—'}.</p>}</div>}<label>Descrição<input autoFocus required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder={receivable ? 'Ex.: Serviço prestado' : 'Ex.: Compra no cartão'}/></label>{receivable ? <><label>Valor a receber<CurrencyInput required value={form.value} onValueChange={value => setForm({ ...form, value })} placeholder="R$ 0,00"/></label><label>Quando esse valor entra?<select value={form.receivableMode} onChange={event => setForm({ ...form, receivableMode: event.target.value })}><option value="once">Uma única vez</option><option value="recurring">Todos os meses</option></select></label>{form.receivableMode === 'once' && <label>Mês de entrada<input required type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label>}</> : <><div className="form-row"><label>{form.installments ? 'Valor de cada parcela' : 'Valor a pagar'}<CurrencyInput required value={form.value} onValueChange={value => setForm({ ...form, value })} placeholder="R$ 0,00"/></label><label>Tipo<select required value={form.type} onChange={event => setForm({ ...form, type: event.target.value })}>{types.map(type => <option key={type.id} value={type.name}>{type.name}</option>)}</select></label></div><div className="form-row"><label>Categoria<select required value={form.category} onChange={event => setForm({ ...form, category: event.target.value })}>{categories.map(category => <option key={category.id} value={category.name}>{category.name}</option>)}</select></label><label>Responsável<select value={form.responsible} onChange={event => setForm({ ...form, responsible: event.target.value })}><option value="">Não definido</option>{people.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label></div><div className="form-row"><label>Quantidade de parcelas <small>{creditCard ? '(mínimo 1)' : '(vazio = recorrente)'}</small><input min="1" max="360" type="number" value={form.installments} onChange={event => setForm({ ...form, installments: event.target.value })} placeholder="Ex.: 12"/></label><label>Primeira parcela<input disabled={!form.installments} required={Boolean(form.installments)} type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label></div></>}<p className="modal-note">Contas a pagar reduzem o saldo. Contas a receber podem entrar em um único mês escolhido ou se repetir todos os meses.</p><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary">{initialBill ? 'Salvar alterações' : 'Salvar lançamento'}</button></div></form></Modal>
}
