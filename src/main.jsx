...
function BillModal({ people, categories, types, initialPeriod, initialBill, forceCreditCard, onClose, onSave }) {
  ...
  const submit = event => {
    ...
    onSave({ ...form, id: initialBill?.id || crypto.randomUUID(), cardName: creditCard ? form.cardName.trim() : '', flow: creditCard ? 'payable' : form.flow, isCreditCard: creditCard, value: Number(form.value), dueDay: receivable ? 1 : Number(form.dueDay), type: receivable ? 'Recebimento' : form.type, category: receivable ? 'Receitas' : form.category, responsible: receivable ? '' : form.responsible, installments: receivable ? (form.receivableMode === 'recurring' ? null : 1) : form.installments ? Number(form.installments) : null, startPeriod: receivable ? (form.receivableMode === 'recurring' ? null : form.startPeriod || initialPeriod) : form.installments ? form.startPeriod : null, status: initialBill?.status || 'pending' })
  }
