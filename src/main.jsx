...
function AuditLogs({ finance }) {
  const labels = { name: 'Descrição', value: 'Valor', dueDay: 'Vencimento', type: 'Tipo', category: 'Categoria', responsible: 'Responsável', installments: 'Parcelas', startPeriod: 'Primeira parcela', flow: 'Fluxo', isCreditCard: 'Lançamento no cartão' }
...
function CreditCardBills({ finance, openModal }) {
  ...
  return <section className="content"><div className="page-title"><div><p className="eyebrow">CARTÃO DE CRÉDITO</p><h1>Compras no cartão</h1><p className="sub">Acompanhe as faturas e parcelas que ainda comprometem seu orçamento.</p></div><button className="primary" onClick={() => openModal('card-bill')}><Plus size={18}/>Nova compra no cartão</button></div><div className="cards credit-summary"><Metric label="Fatura do mês" value={money(total)} icon={<CreditCard/>} tint="coral" detail={`${bills.length} lançamento${bills.length !== 1 ? 's' : ''} no cartão`}/><Metric label="Parcelas ativas" value={String(bills.filter(bill => bill.installments).length)} icon={<CalendarDays/>} tint="purple" detail="Incluídas na projeção"/></div><article className="panel table-panel credit-table"><div className="table-head"><span>DESCRIÇÃO</span><span>TIPO</span><span>VENCIMENTO</span><span>RESPONSÁVEL</span><span>VALOR</span><span>STATUS</span><span/><span/></div>{bills.map(bill => { const person = finance.people.find(item => item.id === bill.responsible); return <div className="table-row" key={bill.id}><div><b>{bill.name}</b><small>{bill.category}{bill.installments ? ` · ${bill.installments} parcela${bill.installments > 1 ? 's' : ''}` : ''}</small></div><span className={`tag ${bill.type === 'Fixa' ? 'fixed' : 'variable'}`}>{bill.type}</span><span>Dia {bill.dueDay}</span><span>{person?.name || '—'}</span><strong>{money(bill.value)}</strong><button className={bill.status === 'paid' ? 'status paid-status clickable' : 'status clickable'} onClick={() => finance.toggleBill(bill, period)}>{bill.status === 'paid' ? 'Pago' : 'Pendente'}</button><button className="edit" title="Editar lançamento" onClick={() => openModal({ type: 'edit-bill', bill })}><Pencil size={15}/></button><button className="delete" onClick={() => finance.remove('bills', bill.id)}><Trash2 size={16}/></button></div> })}{!bills.length && <Empty text="Nenhuma compra no cartão para este mês."/>}</article></section>
}
