...
function App() {
  ...
  return <div className="app-shell">
    ...
    {modal === 'card-bill' && <BillModal people={finance.people} categories={finance.categories} types={finance.types} initialPeriod={periodKey(current)} forceCreditCard onClose={() => setModal(null)} onSave={async bill => { const error = await finance.addBill(bill); if (!error) setModal(null); return error }} />}
...
function BillModal({ people, categories, types, initialPeriod, initialBill, forceCreditCard, onClose, onSave }) {
  ...
  const submit = async event => {
    ...
    const error = await onSave({ ...form, id: initialBill?.id || crypto.randomUUID(), cardName: creditCard ? form.cardName.trim() : '', flow: creditCard ? 'payable' : form.flow, isCreditCard: creditCard, value: Number(form.value), dueDay: receivable ? 1 : Number(form.dueDay), type: receivable ? 'Recebimento' : form.type, category: receivable ? 'Receitas' : form.category, responsible: receivable ? '' : form.responsible, installments: receivable ? (form.receivableMode === 'recurring' ? null : 1) : creditCard ? Math.max(1, Number(form.installments) || 1) : form.installments ? Number(form.installments) : null, startPeriod: receivable ? (form.receivableMode === 'recurring' ? null : form.startPeriod || initialPeriod) : creditCard ? form.startPeriod || initialPeriod : form.installments ? form.startPeriod : null, status: initialBill?.status || 'pending' })
    if (error) window.alert(error)
  }
  const flowField = <><label>Fluxo<select disabled={creditCard} value={creditCard ? 'payable' : form.flow} onChange={event => setForm({ ...form, flow: event.target.value, receivableMode: event.target.value === 'receivable' ? 'once' : form.receivableMode, installments: event.target.value === 'receivable' ? 1 : '', startPeriod: event.target.value === 'receivable' ? initialPeriod : form.startPeriod })}><option value="payable">Conta a pagar</option><option value="receivable">Conta a receber</option></select></label>{creditCard && <label>Cartão<input required value={form.cardName} onChange={event => setForm({ ...form, cardName: event.target.value })} placeholder="Ex.: Nubank"/></label>}</>
