...
const seed = {
  ...
  bills: [
    ...
    { id: 'b4', name: 'Cartão de crédito', value: 780, dueDay: 15, type: 'Flutuante', category: 'Financeiro', responsible: 'p1', status: 'pending', isCreditCard: true },
...
function useFinance() {
  ...
  const updateBill = async bill => {
    ...
    const payload = { name: updatedBill.name, value: updatedBill.value, due_day: updatedBill.dueDay, type: updatedBill.type, category: updatedBill.category, responsible: updatedBill.responsible, installments: updatedBill.installments || null, start_period: updatedBill.startPeriod || null, flow: updatedBill.flow }
...
function App() {
  ...
  const nav = [{ id: 'home', label: 'Visão geral', icon: Home }, { id: 'people', label: 'Cadastros', icon: Users }, { id: 'bills', label: 'Lançamentos', icon: LayoutList }, { id: 'categories', label: 'Categorias e tipos', icon: Tags }, { id: 'logs', label: 'Histórico', icon: History }]
...
function AuditLogs({ finance }) {
  const labels = { name: 'Descrição', value: 'Valor', dueDay: 'Vencimento', type: 'Tipo', category: 'Categoria', responsible: 'Responsável', installments: 'Parcelas', startPeriod: 'Primeira parcela', flow: 'Fluxo' }
...
function Bills({ finance, openModal }) {
  ...
  const bills = finance.bills.filter(bill => isBillActiveInPeriod(bill, period)).map(b => ({ ...b, status: finance.getStatus(b, period) })).filter(bill => filter === 'Todos' || filter === 'A pagar' && isPayable(bill) || filter === 'A receber' && !isPayable(bill) || bill.type === filter)
...
function BillModal({ people, categories, types, initialPeriod, initialBill, onClose, onSave }) {
  const [form, setForm] = useState(() => initialBill ? { name: initialBill.name, value: initialBill.value, dueDay: String(initialBill.dueDay), type: initialBill.type, category: initialBill.category, responsible: initialBill.responsible || '', installments: initialBill.installments || '', startPeriod: initialBill.startPeriod || initialPeriod, flow: initialBill.flow || 'payable' } : { name: '', value: '', dueDay: '10', type: types[0]?.name || '', category: categories[0]?.name || '', responsible: people[0]?.id || '', installments: '', startPeriod: initialPeriod, flow: 'payable' })
  const receivable = form.flow === 'receivable'
  const submit = event => {
    event.preventDefault()
    if (!form.name || !form.value || !form.type || !form.category) return
    onSave({ ...form, id: initialBill?.id || crypto.randomUUID(), value: Number(form.value), dueDay: Number(form.dueDay), installments: receivable ? 1 : form.installments ? Number(form.installments) : null, startPeriod: receivable ? form.startPeriod || initialPeriod : form.installments ? form.startPeriod : null, status: initialBill?.status || 'pending' })
  }
  return <Modal title={initialBill ? 'Editar lançamento' : 'Novo lançamento'} onClose={onClose}><form onSubmit={submit}><div className="form-row"><label>Fluxo<select value={form.flow} onChange={event => setForm({ ...form, flow: event.target.value, installments: event.target.value === 'receivable' ? 1 : '', startPeriod: event.target.value === 'receivable' ? initialPeriod : form.startPeriod })}><option value="payable">Conta a pagar</option><option value="receivable">Conta a receber</option></select></label><label>Vencimento<input required min="1" max="31" type="number" value={form.dueDay} onChange={event => setForm({ ...form, dueDay: event.target.value })}/></label></div><label>Descrição<input autoFocus required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder={receivable ? 'Ex.: Serviço prestado' : 'Ex.: Compra no cartão'}/></label><div className="form-row"><label>{form.installments && !receivable ? 'Valor de cada parcela' : receivable ? 'Valor a receber' : 'Valor a pagar'}<CurrencyInput required value={form.value} onValueChange={value => setForm({ ...form, value })} placeholder="R$ 0,00"/></label><label>Tipo<select required value={form.type} onChange={event => setForm({ ...form, type: event.target.value })}>{types.map(type => <option key={type.id} value={type.name}>{type.name}</option>)}</select></label></div><div className="form-row"><label>Categoria<select required value={form.category} onChange={event => setForm({ ...form, category: event.target.value })}>{categories.map(category => <option key={category.id} value={category.name}>{category.name}</option>)}</select></label><label>Responsável<select value={form.responsible} onChange={event => setForm({ ...form, responsible: event.target.value })}><option value="">Não definido</option>{people.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label></div>{!receivable && <div className="form-row"><label>Quantidade de parcelas <small>(vazio = recorrente)</small><input min="1" max="360" type="number" value={form.installments} onChange={event => setForm({ ...form, installments: event.target.value })} placeholder="Ex.: 12"/></label><label>Primeira parcela<input disabled={!form.installments} required={Boolean(form.installments)} type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label></div>}<p className="modal-note">Contas a pagar reduzem o saldo. Contas a receber são valores únicos e entram como receita somente no mês selecionado.</p><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary">{initialBill ? 'Salvar alterações' : 'Salvar lançamento'}</button></div></form></Modal>
}
