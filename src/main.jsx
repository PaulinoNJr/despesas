...
const isBillActiveInPeriod = (bill, period) => {
  ...
  const start = bill.startPeriod ? periodDate(bill.startPeriod) : null
  // Parcelas permanecem ativas apenas durante a quantidade informada.
  if (bill.installments && start) {
  ...
  // Um lançamento pontual pertence exclusivamente ao seu mês de referência.
  if (bill.isRecurring === false) return bill.startPeriod === period
  // Lançamentos recorrentes começam no mês escolhido e se repetem dali em diante.
  if (bill.isRecurring === true && start) return target >= start
  // Mantém os lançamentos antigos, que não tinham mês de referência, recorrentes.
  return true
}
...
const seed = {
  ...
  cards: [{ id: 'cc1', name: 'Cartão de crédito', dueDay: 15 }],
  bills: [
    ...
    { id: 'b4', name: 'Compras anteriores', value: 780, dueDay: 15, type: 'Flutuante', category: 'Financeiro', responsible: 'p1', status: 'pending', isCreditCard: true, cardId: 'cc1', cardName: 'Cartão de crédito' },
    { id: 'b5', name: 'Academia', value: 99.9, dueDay: 20, type: 'Fixa', category: 'Pessoal', responsible: 'p2', status: 'pending' },
  ]
}

const legacyCards = bills => Array.from(new Map((bills || []).filter(isCreditCardBill).map(bill => [bill.cardName || 'Cartão de crédito', { id: `legacy-card-${bill.cardName || 'cartao-de-credito'}`, name: bill.cardName || 'Cartão de crédito', dueDay: bill.dueDay || 1 }])).values())
const normalizeFinanceData = saved => {
  const cards = saved.cards?.length ? saved.cards : legacyCards(saved.bills)
  return { ...saved, cards, bills: (saved.bills || []).map(bill => bill.isCreditCard && !bill.cardId ? { ...bill, cardId: cards.find(card => card.name === bill.cardName)?.id || null } : bill), payments: saved.payments || {}, logs: saved.logs || [], incomes: saved.incomes || [], categories: saved.categories || seed.categories, types: saved.types || seed.types }
}
...
function useFinance() {
  const [data, setData] = useState(() => {
    if (supabase) return { people: [], incomes: [], categories: [], types: [], cards: [], bills: [], payments: {}, logs: [] }
    try {
      const saved = JSON.parse(localStorage.getItem('conta-clara-data'))
      return saved ? normalizeFinanceData(saved) : { ...seed, payments: {}, logs: [] }
  ...
  useEffect(() => {
    ...
    let active = true
    const load = async () => {
      const [people, bills, payments, incomes, categoryResult, typeResult, auditLogs, invoiceResult, cardResult] = await Promise.all([supabase.from('people').select('*').order('created_at'), supabase.from('bills').select('*').order('due_day'), supabase.from('bill_payments').select('*'), supabase.from('income_payments').select('*').order('pay_day'), supabase.from('expense_categories').select('*').order('name'), supabase.from('expense_types').select('*').order('name'), supabase.from('audit_logs').select('*').order('created_at', { ascending: false }), supabase.from('credit_card_invoices').select('*').order('created_at', { ascending: false }), supabase.from('credit_cards').select('*').order('name')])
      if (active && !people.error && !bills.error && !payments.error && !incomes.error && !categoryResult.error && !typeResult.error && !auditLogs.error && !invoiceResult.error && !cardResult.error) {
        ...
        setData({ people: people.data.map(p => ({ id: p.id, name: p.name, color: p.color })), incomes: incomes.data.map(i => ({ id: i.id, personId: i.person_id, value: Number(i.value), payDay: i.pay_day })), categories, types, cards: cardResult.data.map(card => ({ id: card.id, name: card.name, dueDay: card.due_day })), bills: bills.data.map(b => ({ id: b.id, name: b.name, value: Number(b.value), dueDay: b.due_day, type: b.type, category: b.category, responsible: b.responsible, installments: b.installments, startPeriod: b.start_period, isRecurring: b.is_recurring !== false, flow: b.flow || 'payable', isCreditCard: b.is_credit_card || false, cardId: b.card_id || null, cardName: b.card_name || '', cardInvoiceId: b.card_invoice_id || null })), payments: paymentMap, logs: auditLogs.data.map(log => ({ id: log.id, entityId: log.entity_id, action: log.action, changes: log.changes, createdAt: log.created_at })) })
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      ...
      setUser(session?.user || null)
      if (session) load()
      else { setRemote(false); setData({ people: [], incomes: [], categories: [], types: [], cards: [], bills: [], payments: {}, logs: [] }); setFamily({ members: [], invites: [] }); setInvoices([]) }
      setAuthReady(true)
    })
  ...
  const addCard = async card => {
    ...
    if (!supabase) { setData(next); localStorage.setItem('conta-clara-data', JSON.stringify(next)); return '' }
    const familyId = family.members.find(member => member.user_id === user?.id)?.family_id
    if (!familyId) return 'Não foi possível identificar a família para este cartão.'
    const { error } = await supabase.from('credit_cards').insert({ id: card.id, family_id: familyId, name: card.name, due_day: card.dueDay })
    if (error) return error.message || 'Não foi possível cadastrar o cartão.'
    setData(next)
    return ''
  }
  const removeCard = async cardId => {
    const linkedBills = data.bills.filter(bill => bill.cardId === cardId)
    if (linkedBills.length) return `Não é possível excluir este cartão porque ele possui ${linkedBills.length} lançamento${linkedBills.length === 1 ? '' : 's'} vinculado${linkedBills.length === 1 ? '' : 's'}.`
    const next = { ...data, cards: data.cards.filter(card => card.id !== cardId) }
    if (!supabase) { setData(next); localStorage.setItem('conta-clara-data', JSON.stringify(next)); return '' }
    const { error } = await supabase.from('credit_cards').delete().eq('id', cardId)
    if (error) return error.message || 'Não foi possível excluir o cartão.'
    setData(next)
    return ''
  }
  const addBill = async bill => {
    const next = { ...data, bills: [...data.bills, bill] }
    const payload = { id: bill.id, name: bill.name, value: bill.value, due_day: bill.dueDay, type: bill.type, category: bill.category, responsible: bill.responsible || null, installments: bill.installments || null, start_period: bill.startPeriod || null, is_recurring: bill.isRecurring !== false, flow: bill.flow || 'payable', is_credit_card: Boolean(bill.isCreditCard), card_id: bill.cardId || null, card_name: bill.isCreditCard ? bill.cardName : null, card_invoice_id: bill.cardInvoiceId || null }
    if (!supabase) { setData(next); localStorage.setItem('conta-clara-data', JSON.stringify(next)); return '' }
    const { error } = await supabase.from('bills').insert(payload)
    if (error) return `Não foi possível salvar este lançamento: ${error.message}`
    setData(next)
    return ''
  }
  const updateBill = async bill => {
    ...
    const before = { ...existingBill, flow: existingBill.flow || 'payable', installments: existingBill.installments || null, startPeriod: existingBill.startPeriod || null, isRecurring: existingBill.isRecurring !== false, isCreditCard: Boolean(existingBill.isCreditCard), cardName: existingBill.cardName || '' }
    const updatedBill = { ...bill, flow: bill.flow || 'payable', isRecurring: bill.isRecurring !== false, isCreditCard: Boolean(bill.isCreditCard), cardName: bill.isCreditCard ? bill.cardName : '' }
  ...
  const importCardInvoice = async ({ invoice, bills }) => {
    ...
    const payload = nextBills.map(bill => ({ id: bill.id, name: bill.name, value: bill.value, due_day: bill.dueDay, type: bill.type, category: bill.category, responsible: bill.responsible || null, installments: bill.installments || null, start_period: bill.startPeriod || null, is_recurring: false, flow: 'payable', is_credit_card: true, card_id: bill.cardId || null, card_name: bill.cardName, card_invoice_id: invoice.id }))
  ...
  return { ...data, invoices, family, remote, user, authReady, connectionError, preferences, signIn, signInWithPasskey, signOut, registerPasskey, removePasskey, changePassword, updateDisplayName, updatePreferences, inviteFamilyMember, cancelFamilyInvite, getFamilyInvitation, acceptFamilyInvitation, addPerson, addIncome, addCategory, addType, addCard, removeCard, addBill, updateBill, importCardInvoice, removeCardInvoice, toggleBill, getStatus, remove }
}
  ...
  return <div className="app-shell">
    ...
    {modal === 'bill' && (
      <BillModal
        ...
        onSave={async bill => { const error = await finance.addBill(bill); if (!error) setModal(null); return error }}
      />
    )}
    {modal?.type === 'edit-bill' && <BillModal people={finance.people} categories={finance.categories} types={finance.types} cards={finance.cards} initialPeriod={periodKey(current)} initialBill={modal.bill} onClose={() => setModal(null)} onSave={bill => { finance.updateBill(bill); setModal(null) }} />}
    {modal === 'card-bill' && <BillModal people={finance.people} categories={finance.categories} types={finance.types} cards={finance.cards} initialPeriod={periodKey(current)} forceCreditCard onClose={() => setModal(null)} onSave={async bill => { const error = await finance.addBill(bill); if (!error) setModal(null); return error }} />}
    {modal === 'credit-card' && <CreditCardModal onClose={() => setModal(null)} onSave={async card => { const error = await finance.addCard(card); if (!error) setModal(null); return error }} />}
    {modal?.type === 'review-card-invoice' && <CardInvoiceReviewModal finance={finance} initialReview={modal.review} onClose={() => setModal(null)} />}
...
function Dashboard({ finance, displayName, current, offset, setOffset, openModal }) {
  ...
  const projection = Array.from({ length: finance.preferences.projectionMonths }, (_, index) => {
    ...
    const activeCardInstallments = monthBills.filter(bill => isPayable(bill) && isCreditCardBill(bill))
    ...
    return { date, value: accumulatedBalance, cardInstallments: activeCardInstallments.reduce((sum, bill) => sum + bill.value, 0), cardInstallmentCount: activeCardInstallments.length }
  })
  ...
  return <section className="content">
    ...
    <div className="cards">
      <Metric label="Receitas do mês" value={money(income)} icon={<TrendingUp/>} tint="purple" detail={incomeDetail}/>
...
function RowActions({ label, onEdit, onDelete }) {
  ...
  return <div className="row-actions" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false) }}>
    <button type="button" className="action-menu" aria-label={`Ações para ${label}`} aria-expanded={open} onClick={() => setOpen(value => !value)}><MoreHorizontal size={18}/></button>
    {open && <div className="action-popover"><button type="button" onClick={onEdit}><Pencil size={15}/>Editar</button><button type="button" className="danger" onClick={onDelete}><Trash2 size={15}/>Excluir</button></div>}
  </div>
}
...
function AuditLogs({ finance }) {
  const labels = { name: 'Descrição', value: 'Valor', dueDay: 'Vencimento', type: 'Tipo', category: 'Categoria', responsible: 'Responsável', installments: 'Parcelas', startPeriod: 'Mês de referência', isRecurring: 'Repetição', flow: 'Fluxo', isCreditCard: 'Lançamento no cartão', cardName: 'Cartão' }
  const formatValue = (key, value) => {
    if (key === 'value') return money(value)
    if (key === 'dueDay') return `Dia ${value}`
    if (key === 'installments') return value || 'Recorrente'
    if (key === 'isRecurring') return value ? 'Todos os meses' : 'Somente no mês selecionado'
...
function Bills({ finance, openModal }) {
  ...
  const bills = finance.bills
    .filter(bill => !isCreditCardBill(bill))
    .filter(bill => isBillActiveInPeriod(bill, period))
    .map(bill => {
      const scheduledPeriod = period
      return { ...bill, scheduledPeriod, status: finance.getStatus(bill, scheduledPeriod) }
    })
    .filter(bill => filter === 'Todos' || filter === 'A pagar' && isPayable(bill) || filter === 'A receber' && !isPayable(bill) || bill.type === filter)
    .sort((a, b) => (a.scheduledPeriod || period).localeCompare(b.scheduledPeriod || period) || a.dueDay - b.dueDay)
  return <section className="content"><div className="page-title"><div><p className="eyebrow">LANÇAMENTOS</p><h1>Contas a pagar e a receber</h1><p className="sub">Registre despesas, entradas previstas e acompanhe cada vencimento.</p></div><button className="primary" onClick={() => openModal('bill')}><Plus size={18}/>Novo lançamento</button></div><div className="filters">{['Todos', 'A pagar', 'A receber', ...finance.types.map(type => type.name)].map(item => <button key={item} onClick={() => setFilter(item)} className={filter === item ? 'selected' : ''}>{item === 'Todos' ? 'Todos os lançamentos' : item}</button>)}</div><article className="panel table-panel"><div className="table-head"><span>DESCRIÇÃO</span><span>TIPO</span><span>VENCIMENTO</span><span>RESPONSÁVEL</span><span>VALOR</span><span>STATUS</span><span/></div>{bills.map(b => { const p = finance.people.find(x => x.id === b.responsible); const receivable = !isPayable(b); const scheduledDate = receivable && b.startPeriod ? `${months[periodDate(b.startPeriod).getMonth()].slice(0, 3)}. ${periodDate(b.startPeriod).getFullYear()}` : `Dia ${b.dueDay}`; return <div className="table-row" key={b.id}><div><b>{b.name}</b><small>{receivable ? 'A receber' : 'A pagar'} · {b.category}</small></div><span className={`tag ${receivable ? 'receivable' : b.type === 'Fixa' ? 'fixed' : 'variable'}`}>{receivable ? 'Receber' : b.type}</span><span>{scheduledDate}</span><span>{p?.name || '—'}</span><strong className={receivable ? 'income-value' : ''}>{money(b.value)}</strong><button className={b.status === 'paid' ? 'status paid-status clickable' : 'status clickable'} onClick={() => finance.toggleBill(b, b.scheduledPeriod)}>{b.status === 'paid' ? (receivable ? 'Recebido' : 'Pago') : 'Pendente'}</button><RowActions label={b.name} onEdit={() => openModal({ type: 'edit-bill', bill: b })} onDelete={() => finance.remove('bills', b.id)}/></div>})}{bills.length === 0 && <Empty text="Nenhum lançamento neste filtro."/>}</article></section>
}
...
function CreditCardBills({ finance, openModal }) {
  ...
  const [selectingImportCard, setSelectingImportCard] = useState(false)
  const [removingCardId, setRemovingCardId] = useState('')
  ...
  const cards = finance.cards.map(card => {
    const cardBills = bills.filter(bill => bill.cardId === card.id || !bill.cardId && bill.cardName === card.name)
    return { ...card, bills: cardBills, total: cardBills.reduce((sum, bill) => sum + bill.value, 0) }
  })
  const readInvoice = async event => {
    const file = event.target.files?.[0]
    event.target.value = ''
    if (!file || !importCardId) return
    setImporting(true); setImportMessage('Lendo sua fatura…')
    try {
      await new Promise(resolve => requestAnimationFrame(resolve))
      const { parseCreditCardInvoicePdf } = await import('./card-invoice-parser')
      const invoice = await parseCreditCardInvoicePdf(file, { onProgress: progress => setImportMessage(progress.label) })
      openModal({ type: 'review-card-invoice', review: { invoice, fileName: file.name, cardId: importCardId } })
      setImportCardId('')
      setImportMessage('')
  ...
  const startImport = () => {
    if (!finance.cards.length) return setImportMessage('Cadastre um cartão antes de importar uma fatura.')
    setImportCardId(finance.cards[0].id)
    setSelectingImportCard(true)
  }
  const chooseImportCard = event => {
    event.preventDefault()
    if (!importCardId) return
    setSelectingImportCard(false)
  ...
  const removeRegisteredCard = async card => {
    if (!window.confirm(`Excluir o cartão ${card.name}? Esta ação não pode ser desfeita.`)) return
    setRemovingCardId(card.id)
    const error = await finance.removeCard(card.id)
    setRemovingCardId('')
    setImportMessage(error || 'Cartão excluído com sucesso.')
  }
  return <section className="content"><input className="visually-hidden" ref={fileInputRef} type="file" accept="application/pdf,.pdf" onChange={readInvoice}/><div className="page-title"><div><p className="eyebrow">CARTÃO DE CRÉDITO</p><h1>Cartões e compras</h1><p className="sub">Cadastre o cartão e seu vencimento antes de lançar as compras.</p></div><div className="card-actions"><button className="secondary" disabled={importing} onClick={startImport}><Download size={17}/>{importing ? 'Lendo fatura…' : 'Importar fatura'}</button><button className="secondary" onClick={() => openModal('credit-card')}><Plus size={17}/>Cadastrar cartão</button><button className="primary" disabled={!cards.length} onClick={() => openModal('card-bill')}><Plus size={18}/>Nova compra</button></div></div>{importMessage && <div className={importing ? 'invoice-inline-loading' : 'login-error'}>{importing && <span className="mini-loader"/>}{importMessage}</div>}{selectingImportCard && <Modal title="Importar fatura" onClose={() => setSelectingImportCard(false)}><form onSubmit={chooseImportCard}><label>Para qual cartão é esta fatura?<select autoFocus required value={importCardId} onChange={event => setImportCardId(event.target.value)}>{finance.cards.map(card => <option key={card.id} value={card.id}>{card.name} · vence dia {card.dueDay}</option>)}</select></label><p className="modal-note">A fatura e todos os lançamentos identificados serão vinculados a este cartão.</p><div className="modal-actions"><button type="button" className="secondary" onClick={() => setSelectingImportCard(false)}>Cancelar</button><button className="primary">Selecionar arquivo</button></div></form></Modal>}{!cards.length && <div className="info-strip"><CreditCard size={20}/><span>Cadastre um cartão com o dia de vencimento para liberar novos lançamentos e importações.</span></div>}<div className="cards credit-summary"><Metric label="Fatura total" value={money(total)} icon={<CreditCard/>} tint="coral" detail={`${bills.length} lançamento${bills.length !== 1 ? 's' : ''} neste mês`}/><Metric label="Cartões cadastrados" value={String(cards.length)} icon={<CalendarDays/>} tint="purple" detail="Prontos para receber compras"/></div><div className="card-invoice-list">{cards.map(card => <article className="panel card-invoice" key={card.id}><div><span>FATURA · {card.name}</span><strong>{money(card.total)}</strong></div><small>Vencimento no dia {card.dueDay} · {card.bills.length} compra{card.bills.length !== 1 ? 's' : ''} lançada{card.bills.length !== 1 ? 's' : ''}</small><button className="delete-import" disabled={removingCardId === card.id} onClick={() => removeRegisteredCard(card)}><Trash2 size={15}/>{removingCardId === card.id ? 'Excluindo…' : 'Excluir cartão'}</button></article>)}{finance.invoices.map(invoice => <article className="panel card-invoice" key={invoice.id}><div><span>IMPORTAÇÃO · {invoice.cardName}</span><strong>{money(invoice.statementTotal)}</strong></div><small>Vencimento em {dateLabel(invoice.dueDate)}</small><button className="delete-import" disabled={removingInvoiceId === invoice.id} onClick={() => removeImportedInvoice(invoice)}><Trash2 size={15}/>{removingInvoiceId === invoice.id ? 'Removendo…' : 'Remover importação'}</button></article>)}</div><article className="panel table-panel credit-table"><div className="table-head"><span>DESCRIÇÃO</span><span>TIPO</span><span>VENCIMENTO</span><span>RESPONSÁVEL</span><span>VALOR</span><span>STATUS</span><span/><span/></div>{bills.map(bill => { const person = finance.people.find(item => item.id === bill.responsible); const card = finance.cards.find(item => item.id === bill.cardId); return <div className="table-row" key={bill.id}><div><b>{bill.name}</b><small>{card?.name || bill.cardName || 'Cartão de crédito'} · {bill.category}{bill.installments ? ` · ${bill.installments} parcela${bill.installments > 1 ? 's' : ''}` : ''}</small></div><span className={`tag ${bill.type === 'Fixa' ? 'fixed' : 'variable'}`}>{bill.type}</span><span>Dia {bill.dueDay}</span><span>{person?.name || '—'}</span><strong>{money(bill.value)}</strong><button className={bill.status === 'paid' ? 'status paid-status clickable' : 'status clickable'} onClick={() => finance.toggleBill(bill, period)}>{bill.status === 'paid' ? 'Pago' : 'Pendente'}</button><button className="edit" title="Editar lançamento" onClick={() => openModal({ type: 'edit-bill', bill })}><Pencil size={15}/></button><button className="delete" onClick={() => finance.remove('bills', bill.id)}><Trash2 size={16}/></button></div> })}{!bills.length && <Empty text={cards.length ? 'Nenhuma compra no cartão para este mês.' : 'Cadastre um cartão para começar.'}/>}</article></section>
}
...
function CardInvoiceReviewModal({ finance, initialReview, onClose }) {
  ...
  const [message, setMessage] = useState('')
  const [cardId] = useState(() => initialReview?.cardId || '')
  const [saving, setSaving] = useState(false)
  ...
  const confirm = async () => {
    ...
    if (!invoice || !isExact || !card) return
    setSaving(true); setMessage('')
    const dueDate = String(invoice.dueDate || '').split('/').reverse().join('-')
    const savedInvoice = { id: crypto.randomUUID(), cardName: card.name, invoiceKey: String(invoice.invoiceKey || crypto.randomUUID()), statementTotal: invoiceTotal, dueDate, sourceFileName: String(initialReview?.fileName || '') }
    const bills = selectedEntries.filter(entry => Number(entry.value) !== 0).map(entry => ({ id: crypto.randomUUID(), name: String(entry.description || 'Compra no cartão'), value: Number(entry.value), dueDay: card.dueDay, type: entry.installmentsLeft > 1 ? 'Parcelada' : 'Fatura importada', category: String(entry.category || 'Outros'), responsible: '', installments: Math.max(1, Number(entry.installmentsLeft) || 1), startPeriod: String(invoice.period || currentPeriod()), flow: 'payable', isCreditCard: true, cardId: card.id, cardName: card.name, status: 'pending' }))
  ...
  const selectedCard = finance.cards.find(card => card.id === cardId)
  return <Modal title="Conferir fatura do cartão" onClose={onClose}><div className="invoice-import"><div className="invoice-summary"><div><span>IDENTIFICADOR</span><strong>{String(invoice.invoiceKey || 'Fatura importada')}</strong></div><div><span>VENCIMENTO</span><strong>{String(invoice.dueDate || 'Não identificado')}</strong></div><div><span>TOTAL DA FATURA</span><strong>{money(invoiceTotal)}</strong></div></div><p className="modal-note">Esta fatura será vinculada a <strong>{selectedCard?.name || 'um cartão indisponível'}</strong>{selectedCard ? `, com vencimento no dia ${selectedCard.dueDay}.` : '.'}</p><div className="invoice-review-head"><strong>Lançamentos identificados</strong><span className={isExact ? 'exact' : 'not-exact'}>{isExact ? 'Total confere' : `Diferença de ${money(invoiceTotal - selectedTotal)}`}</span></div><div className="invoice-entry-list">{entries.map(entry => <div className="invoice-entry" key={entry.id}><input type="checkbox" checked={Boolean(entry.included)} onChange={event => updateEntry(entry.id, { included: event.target.checked })}/><div><input value={String(entry.description)} onChange={event => updateEntry(entry.id, { description: event.target.value })}/><small>{entry.date || 'Ajuste da fatura'}{entry.installmentLabel ? ` · parcela ${entry.installmentLabel}` : ''}</small></div><input className="invoice-value" type="number" step="0.01" value={Number(entry.value) || 0} onChange={event => updateEntry(entry.id, { value: Number(event.target.value) || 0 })}/><label className="invoice-installments">Restam<input type="number" min="1" max="360" value={Math.max(1, Number(entry.installmentsLeft) || 1)} onChange={event => updateEntry(entry.id, { installmentsLeft: Number(event.target.value) || 1 })}/></label></div>)}</div>{message && <div className="login-error">{message}</div>}<p className="modal-note">Revise os lançamentos antes de confirmar a importação.</p><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={!isExact || saving || !selectedCard} onClick={confirm}>{saving ? 'Importando…' : 'Confirmar e importar'}</button></div></div></Modal>
}

function Modal({ title, children, onClose }) { return <div className="modal-backdrop"><div className="modal" role="dialog" aria-modal="true" aria-label={title}><div className="modal-head"><h2>{title}</h2><button className="icon-button" aria-label="Fechar" onClick={onClose}><X size={20}/></button></div>{children}</div></div> }
...
function CreditCardModal({ onClose, onSave }) {
  const [form, setForm] = useState({ name: '', dueDay: '10' })
  const [message, setMessage] = useState('')
  const [saving, setSaving] = useState(false)
  const submit = async event => {
    event.preventDefault()
    ...
    setSaving(true); setMessage('')
    const error = await onSave({ id: crypto.randomUUID(), name: form.name.trim(), dueDay: Number(form.dueDay) })
    setSaving(false)
    if (error) setMessage(error)
  }
  return <Modal title="Cadastrar cartão" onClose={onClose}><form onSubmit={submit}><label>Nome do cartão<input autoFocus required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder="Ex.: Nubank Platinum"/></label><label>Dia de vencimento<input required min="1" max="31" type="number" value={form.dueDay} onChange={event => setForm({ ...form, dueDay: event.target.value })}/></label>{message && <div className="login-error">{message}</div>}<p className="modal-note">Esse vencimento será usado automaticamente em todas as compras vinculadas a este cartão.</p><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Salvando…' : 'Salvar cartão'}</button></div></form></Modal>
}
function BillModal({ people, categories, types, cards = [], initialPeriod, initialBill, forceCreditCard, onClose, onSave }) {
  const [form, setForm] = useState(() => initialBill ? { name: initialBill.name, value: initialBill.value, dueDay: String(initialBill.dueDay), type: initialBill.type, category: initialBill.category, responsible: initialBill.responsible || '', installments: initialBill.isCreditCard ? (initialBill.installments || 1) : initialBill.installments || '', startPeriod: initialBill.startPeriod || initialPeriod, flow: initialBill.flow || 'payable', receivableMode: initialBill.flow === 'receivable' && initialBill.isRecurring !== false ? 'recurring' : 'once', recurrence: initialBill.installments ? 'installments' : initialBill.isRecurring === false ? 'once' : 'recurring', isCreditCard: Boolean(initialBill.isCreditCard), cardId: initialBill.cardId || cards.find(card => card.name === initialBill.cardName)?.id || '', cardInvoiceId: initialBill.cardInvoiceId || null } : { name: '', value: '', dueDay: '10', type: types[0]?.name || '', category: categories[0]?.name || '', responsible: people[0]?.id || '', installments: forceCreditCard ? 1 : '', startPeriod: initialPeriod, flow: 'payable', receivableMode: 'once', recurrence: 'recurring', isCreditCard: Boolean(forceCreditCard), cardId: forceCreditCard ? cards[0]?.id || '' : '', cardInvoiceId: null })
  ...
  const submit = async event => {
    ...
    if (!form.name || !form.value || creditCard && !selectedCard || !receivable && (!form.type || !form.category)) return
    const error = await onSave({ ...form, id: initialBill?.id || crypto.randomUUID(), cardId: creditCard ? selectedCard?.id || null : null, cardName: creditCard ? selectedCard?.name || '' : '', flow: creditCard ? 'payable' : form.flow, isCreditCard: creditCard, isRecurring: receivable ? form.receivableMode === 'recurring' : !creditCard && form.recurrence === 'recurring', value: Number(form.value), dueDay: receivable ? 1 : creditCard ? selectedCard.dueDay : Number(form.dueDay), type: receivable ? 'Recebimento' : form.type, category: receivable ? 'Receitas' : form.category, responsible: receivable ? '' : form.responsible, installments: receivable ? (form.receivableMode === 'recurring' ? null : 1) : creditCard ? Math.max(1, Number(form.installments) || 1) : form.recurrence === 'installments' ? Math.max(1, Number(form.installments) || 1) : form.recurrence === 'once' ? 1 : null, startPeriod: form.startPeriod || initialPeriod, status: initialBill?.status || 'pending' })
    if (error) window.alert(error)
  }
  const flowField = <><label>Fluxo<select disabled={creditCard} value={creditCard ? 'payable' : form.flow} onChange={event => setForm({ ...form, flow: event.target.value, receivableMode: event.target.value === 'receivable' ? 'once' : form.receivableMode, recurrence: event.target.value === 'receivable' ? 'once' : form.recurrence, installments: event.target.value === 'receivable' ? '' : form.installments })}><option value="payable">Conta a pagar</option><option value="receivable">Conta a receber</option></select></label>{creditCard && <label>Cartão<select required value={form.cardId} onChange={event => setForm({ ...form, cardId: event.target.value })}><option value="">Selecione um cartão</option>{cards.map(card => <option key={card.id} value={card.id}>{card.name} · vence dia {card.dueDay}</option>)}</select></label>}</>
  const selectedCard = cards.find(card => card.id === form.cardId)
  return <Modal title={initialBill ? 'Editar lançamento' : creditCard ? 'Nova compra no cartão' : 'Novo lançamento'} onClose={onClose}>
    <form onSubmit={submit}>
      {receivable ? flowField : <div className="form-row">{flowField}{!creditCard && <label>Vencimento<input required min="1" max="31" type="number" value={form.dueDay} onChange={event => setForm({ ...form, dueDay: event.target.value })}/></label>}{creditCard && <p className="modal-note">Vencimento: dia {selectedCard?.dueDay || '—'}.</p>}</div>}
      <label>Descrição<input autoFocus required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder={receivable ? 'Ex.: Serviço prestado' : 'Ex.: Compra no cartão'}/></label>
      {receivable ? <>
        <label>Valor a receber<CurrencyInput required value={form.value} onValueChange={value => setForm({ ...form, value })} placeholder="R$ 0,00"/></label>
        <label>Repetição<select value={form.receivableMode} onChange={event => setForm({ ...form, receivableMode: event.target.value })}><option value="once">Somente neste mês</option><option value="recurring">Todos os meses</option></select></label>
        <label>{form.receivableMode === 'recurring' ? 'A partir de qual mês?' : 'Mês do lançamento'}<input required type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label>
      </> : <>
        <div className="form-row"><label>{form.recurrence === 'installments' || creditCard ? 'Valor de cada parcela' : 'Valor a pagar'}<CurrencyInput required value={form.value} onValueChange={value => setForm({ ...form, value })} placeholder="R$ 0,00"/></label><label>Tipo<select required value={form.type} onChange={event => setForm({ ...form, type: event.target.value })}>{types.map(type => <option key={type.id} value={type.name}>{type.name}</option>)}</select></label></div>
        <div className="form-row"><label>Categoria<select required value={form.category} onChange={event => setForm({ ...form, category: event.target.value })}>{categories.map(category => <option key={category.id} value={category.name}>{category.name}</option>)}</select></label><label>Responsável<select value={form.responsible} onChange={event => setForm({ ...form, responsible: event.target.value })}><option value="">Não definido</option>{people.map(person => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label></div>
        {creditCard ? <div className="form-row"><label>Quantidade de parcelas <small>(mínimo 1)</small><input min="1" max="360" type="number" value={form.installments} onChange={event => setForm({ ...form, installments: event.target.value })} placeholder="Ex.: 12"/></label><label>Primeira parcela<input required type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label></div> : <div className="form-row"><label>Repetição<select value={form.recurrence} onChange={event => setForm({ ...form, recurrence: event.target.value, installments: event.target.value === 'installments' ? form.installments || 2 : '' })}><option value="recurring">Todos os meses</option><option value="once">Somente neste mês</option><option value="installments">Parcelado</option></select></label>{form.recurrence === 'installments' && <label>Quantidade de parcelas<input required min="1" max="360" type="number" value={form.installments} onChange={event => setForm({ ...form, installments: event.target.value })} placeholder="Ex.: 12"/></label>}<label>{form.recurrence === 'recurring' ? 'A partir de qual mês?' : form.recurrence === 'installments' ? 'Primeira parcela' : 'Mês do lançamento'}<input required type="month" value={form.startPeriod} onChange={event => setForm({ ...form, startPeriod: event.target.value })}/></label></div>}
      </>}
      <p className="modal-note">Todo lançamento possui um mês de referência. Os recorrentes são incluídos a partir desse mês; os pontuais entram somente no mês selecionado.</p>
      <div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary">{initialBill ? 'Salvar alterações' : 'Salvar lançamento'}</button></div>
    </form>
  </Modal>
}
